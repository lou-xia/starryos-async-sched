...
use axlog::{info, warn};
use axsync::Mutex;
use asynctask::TrapFrame;
use starry_signal::{SignalSet, SignalStack, Signo};
...
impl SignalModule {
    ...
    pub fn set_exit_signal(&mut self, signal: Signo) {
        self.exit_signal = Some(signal);
    }

    pub fn get_exit_signal(&self) -> Option<Signo> {
        self.exit_signal
    }
}
...
/// 处理 Terminate 类型的信号
async fn terminate_process(signal: Signo, info: Option<SigInfo>) {
    let current_task = current_task();
...
pub async fn handle_signals() {
    ...
    let mut sp = if action.sa_flags.contains(SigActionFlags::SA_ONSTACK)
        && signal_module.alternate_stack.flags != starry_signal::ucontext::SS_DISABLE
    {
    ...
    // 若带有SIG_INFO参数，则函数原型为fn(sig: Signo, info: &SigInfo, ucontext: &mut UContext)
